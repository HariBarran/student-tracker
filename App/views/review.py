...
def update_review_action():
    ...
    if(is_clean==False):
        return message 
    if(update_review(data['review_id'], data['upvote'], data['downvote'])):
        return jsonify({'message': f"review updated for id: {data['review_id']} "})
    else:
        return jsonify({'message': f"FAILED review updated for review_id: {data['review_id']} "})
